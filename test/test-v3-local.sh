...
# Test script for v3 API locally (for Docker testing)
# Tests the v3 cover image generation endpoint locally

LOCAL_URL="${LOCAL_URL:-http://localhost:3000}"
AUTH_TOKEN="stabledash_cover_gen_2025_secure_xyz789"

echo "🏠 Testing Stabledash Cover Generator v3 locally..."
echo "📍 URL: $LOCAL_URL"
echo "📁 Output will be saved to: ./test/generated-cover-v3-local-custom.jpg"
echo ""
...
curl -X POST \
  ...
  "$LOCAL_URL/api/generate-cover-image-v3" \
  --output ./test/generated-cover-v3-local-custom.jpg \
...
if [ $? -eq 0 ]; then
  ...
  echo "📸 Check ./test/generated-cover-v3-local-custom.jpg"
  file ./test/generated-cover-v3-local-custom.jpg
else
  echo ""
