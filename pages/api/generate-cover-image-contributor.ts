...
interface ContributorCoverImageRequest {
  background: string
  author_cover_image_url: string
  author_name?: string
  logo?: string
  quality: number
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const body: ContributorCoverImageRequest = req.body

    // Validate required fields
    if (!body.background || !body.author_cover_image_url) {
      return res.status(400).json({
        error: 'Missing required fields: background and author_cover_image_url'
      })
    }
    ...
    if (size !== 'custom' && sizePresets[size]) {
      ...
      // Default to OG size if invalid
      bgWidth = sizePresets.og.width
      bgHeight = sizePresets.og.height
    }
    ...
    // Fetch author cover image and convert to base64
    console.log('Author cover image URL:', body.author_cover_image_url)
    const authorImageResponse = await fetch(body.author_cover_image_url)
    if (!authorImageResponse.ok) {
      return res.status(400).json({
        error: 'Failed to fetch author cover image'
      })
    }
    const authorImageBuffer = await authorImageResponse.arrayBuffer()
    const authorImageBase64 = Buffer.from(authorImageBuffer).toString('base64')
    const authorImageExtension = body.author_cover_image_url.split('.').pop()?.toLowerCase() || 'png'
    const authorImageMimeType = authorImageExtension === 'svg' ? 'image/svg+xml' : `image/${authorImageExtension}`
    ...
    if (body.logo) {
      try {
        ...
        if (logoResponse.ok) {
          ...
          logoMimeType = logoExtension === 'svg' ? 'image/svg+xml' : `image/${logoExtension}`
        }
      } catch (error) {
    ...
    if (useOverlay && SHOW_OVERLAY) {
      try {
        ...
        if (overlayResponse.ok) {
          ...
          overlayBase64 = Buffer.from(overlayBuffer).toString('base64')
        }
      } catch (error) {
    ...
    <head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=PP+Eiko:wght@300;400;500;600;700&display=swap" rel="stylesheet">
        <style>
            @font-face {
                font-family: 'PP Eiko';
                font-style: normal;
                font-weight: 400;
                font-display: swap;
                src: url('https://fonts.gstatic.com/s/ppeiko/v1/PPEiko-Regular.woff2') format('woff2');
            }
            * {
                margin: 0;
                padding: 0;
            ...
            .background {
                ...
                width: 100%;
                height: 100%;
                background-image: url(data:${backgroundMimeType};base64,${backgroundBase64});
            ...
            .overlay-texture {
                ...
                width: 100%;
                height: 100%;
                background-image: url(data:image/png;base64,${overlayBase64});
                background-size: cover;
                background-position: center;
                opacity: 1;
                pointer-events: none;
            ...
            .gradient-overlay {
                ...
                background: linear-gradient(to top, 
                    rgba(0, 0, 0, ${gradientIntensity}) 0%, 
                    rgba(0, 0, 0, ${gradientIntensity * 0.8}) 50%, 
                    rgba(0, 0, 0, 0) 100%);
                pointer-events: none;
            }
            .brand-gradient {
            ...
            .author-name {
                ...
                left: 300px;
                color: white;
                font-family: 'PP Eiko', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
                font-size: 24px;
                font-weight: 400;
                text-align: left;
                pointer-events: none;
    ...
    <body>
        ...
        ${overlayBase64 ? '<div class="overlay-texture"></div>' : ''}
        <div class="gradient-overlay"></div>
        ${brandGradientCSS ? '<div class="brand-gradient"></div>' : ''}
        <div class="author-image-container">
            <img src="data:${authorImageMimeType};base64,${authorImageBase64}" class="author-image" alt="Author" />
        </div>
        ${body.author_name ? `<div class="author-name">${body.author_name}</div>` : ''}
    ...
    console.log('Puppeteer-only rendering completed successfully')
